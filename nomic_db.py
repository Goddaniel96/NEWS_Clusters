...
from sklearn.neighbors import NearestNeighbors # สำหรับ DBSCAN K-distance graph
import warnings
...
if __name__ == "__main__":
    print("--- เริ่มต้นกระบวนการ Clustering ข่าวด้วย Ollama ---")
    ...
    for i, article in enumerate(news_articles):
        ...
        if pd.isna(article) or not isinstance(article, str) or not article.strip():
            # print(f"Skipping empty/invalid article at original index {i}")
            continue
    ...
    print(f"\n[Step 4/6] กำลังทำ Clustering ข่าว...")

    # --- 4.1 Find eps for DBSCAN by use K-distance graph ---
    print("  > กำลังสร้าง K-distance Graph เพื่อช่วยหาค่า 'eps' ที่เหมาะสมสำหรับ DBSCAN...")
    min_samples_to_try_for_eps = 5 # default min_samples for find eps
    neighbors = NearestNeighbors(n_neighbors=min_samples_to_try_for_eps).fit(X)
    distances, indices = neighbors.kneighbors(X)
    distances = np.sort(distances[:, min_samples_to_try_for_eps-1], axis=0)

    plt.figure(figsize=(10, 6))
    plt.plot(distances)
    plt.title(f'K-distance Graph for k = {min_samples_to_try_for_eps-1}')
    plt.xlabel('Points sorted by distance')
    plt.ylabel(f'Distance to {min_samples_to_try_for_eps-1}th nearest neighbor')
    plt.grid(True)
    plt.show()
    print("\n**โปรดดูที่กราฟ K-distance ที่ปรากฏขึ้นมา มองหา 'ข้อศอก' (elbow point) ค่าในแกน Y ที่จุดข้อศอกนั้นคือค่า 'eps' ที่แนะนำ**")
    input("กด Enter เพื่อดำเนินการต่อหลังจากดูกราฟและพิจารณาค่า eps แล้ว...")


    # --- 4.2 RUN DBSCAN Clustering ---
    # Default but maybe tuning
    dbscan_eps = 1  # from k-distance graph
    dbscan_min_samples = 10 # 

    print(f"  > รัน DBSCAN Clustering ด้วย eps={dbscan_eps}, min_samples={dbscan_min_samples}...")
    ...
    print(f"\n[Step 4/6] ผลลัพธ์ DBSCAN Clustering:")
    ...
    # t-SNE เหมาะสำหรับแสดงผลใน 2D
    # ลองปรับ perplexity และ learning_rate หากได้ภาพที่ไม่ดี
    ...
    sns.scatterplot(
        ...
        hue=df_clustered['cluster'], # ใช้ผลลัพธ์จาก DBSCAN clustering
        palette=sns.color_palette("tab10", len(np.unique(df_clustered['cluster']))), # ปรับ palette ตามจำนวนกลุ่มจริง
        legend="full",
        alpha=0.7,
        s=50 # ปรับขนาดจุดให้ใหญ่ขึ้นเล็กน้อย
    )
    plt.title('News Article Clusters (DBSCAN - t-SNE visualization)', fontsize=16)
